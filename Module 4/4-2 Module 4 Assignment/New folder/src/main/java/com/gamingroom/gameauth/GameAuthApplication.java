...
public class GameAuthApplication extends Application<Configuration> {
	...
	public void run(Configuration c, Environment e) throws Exception {
		...
		// FIXME: register GameUserRESTController (based on BasicAuth Security
		// Example)(Fixed)
		...
		e.jersey()
				.register(new AuthDynamicFeature(
						new BasicCredentialAuthFilter.Builder<GameUser>().setAuthenticator(new GameAuthenticator())
								.setAuthorizer(new GameAuthorizer()).setRealm("BASIC-AUTH-REALM").buildAuthFilter()));
		...
		Client client = DemoRESTClient.build("DemoRESTClient");
